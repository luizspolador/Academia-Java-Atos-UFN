...
public class BancoDados implements InterfaceBancoDados {
    ...
    public void exibirDados() throws IOException {
        try {
            PreparedStatement stmt = conn.prepareStatement("SELECT * FROM pessoas");
            ResultSet rs = stmt.executeQuery();
            while (rs.next()) {
                int id = rs.getInt("id");
                ...
                meuLog.logger.info("ID: " + id + ", Nome: " + nome + ", Idade: " + idade);
            }
        } catch (SQLException e) {
            System.out.println("Erro ao exibir dados do banco de dados: " + e.getMessage());
        }
    }
}
